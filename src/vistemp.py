...
class DeepFeatures(torch.nn.Module):
    ...
    def generate_embeddings(self, x):
        ...
        '''
        mu, logvar, logspike = self.model.encode(x)
        return(self.model.reparameterize(mu, logvar, logspike))
...
IMGS_FOLDER = './Outputs/bbbc021/Images'
EMBS_FOLDER = './Outputs/bbbc021/Embeddings'
TB_FOLDER = './Outputs/Tensorboard'
EXPERIMENT_NAME = 'CVSC'
DEVICE = device

DF = DeepFeatures(model = vsc.model, 
...
with torch.no_grad():
    for data, _ in test_loader:
        DF.write_embeddings(x = data.to(DEVICE))
        break
