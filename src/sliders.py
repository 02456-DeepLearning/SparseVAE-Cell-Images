...
# chkpt='./results/checkpoints/convvsc_ConvVSC_cell_1_100_20_0-001_30.pth'
...
class Ui_MainWindow(object):
    ...
    def runInf(self):
        latent_space = np.random.normal(0.0,1.0,args.latent_size)
        latent_space = np.reshape(latent_space,[1,args.latent_size])
        # print(latent_space.shape)
        latent_space = torch.Tensor(latent_space).to(device)
        generation = self.model.decode(latent_space)
        # print(generation.shape)
        generation = generation.cpu()
        generation = generation.detach().numpy()
        generation = np.squeeze(generation)
        # generation = np.reshape(generation, [3, 68, 68])
        generation = np.moveaxis(generation, 0, -1)
        im = Image.fromarray(np.uint8(generation * 255), 'RGB')
        qim = ImageQt(im)
    ...
    def setupUi(self, MainWindow):
        ...
        button = QPushButton('Random initialization', self.graphicLabel)
        button.setToolTip('Randomly initializes each parameter')
        button.move(0,50)
        button.clicked.connect(self.on_click)
        self.textbox = QLineEdit('', self.graphicLabel)
        self.textbox.move(0, 450)
        button2 = QPushButton('Set specific', self.graphicLabel)
        button2.move(0, 475)
    ...
    def on_click(self):
        latent_space = np.random.normal(0.0,1.0,args.latent_size)
        for i in range(len(latent_params)):
            exec('self.horizontalSlider_%d.setValue(%d)' % (i, latent_space[i]*100))

    def on_click2(self):
        latent_space = [float(v) for v in self.textbox.text().split(",")]
        for i in range(len(latent_params)):
            exec('self.horizontalSlider_%d.setValue(%d)' % (i, latent_space[i]*100))
